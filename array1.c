...
//main 시작함수로 시작  
{
	...
	for (int i = 0; i < 10; i++) 
	//반복문인 for문을 사용하여 임의의 변수 i가 0부터 시작해서 10보다 작을때 아래의 코드(line 14 ~ 19)를 실행하고 i의 값에 1을 더함  
	{
	...
	for (int i = 0; i < 10; i++)
	//반복문인 for문을 사용하여 임의의 변수 i가 0부터 시작해서 10보다 작을때 아래의 코드(line 22 ~ 25)를 실행하고 i의 값에 1을 더함  
	{
		printf("%d\n", count[i]);
